...
bool vlog_has_per_clone_state(vlog_node_t v)
{
   switch (vlog_kind(v)) {
   case V_HIER_REF:
      // Resolver writes I_REF / I_IDENT2 onto the node.
      return true;
   case V_BLOCK:
      // Named begin..end blocks become hier-addressable scopes
      // (IEEE 1800-2017 §23.6) and carry their canonical dotted
      // on I_IDENT2.  Unnamed blocks are statement groupings
      // with no per-clone state.
      return vlog_has_ident(v) && vlog_ident(v) != NULL;
   default:
      return false;
   }
}
